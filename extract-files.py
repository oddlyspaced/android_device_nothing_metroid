...
lib_fixups: lib_fixups_user_type = {
    **lib_fixups,
    ...
    (
        # libqapesdk reaches display.config-V12 directly and display.config-V5
        # transitively via liblearningmodule -> libqti-perfextn-intf -> libqti-perfd,
        # so soong sees two versions of one aidl_interface in a single module. Cut the
        # edge: libqti-perfd is shipped as its own blob and still declares V5 itself, so
        # V5 is still built and installed and every consumer still finds libqti-perfd.so
        # at runtime -- only soong's dependency graph changes.
        'libqti-perfd',
        # libqtigefar reaches audio.common-V3 through its own prebuilt chain and
        # audio.common-V4 through source-built libar-pal. libar-pal is built and installed
        # by hardware/qcom-caf/sm8750/audio/pal, so dropping the declared edge leaves the
        # runtime link intact and only removes the second version from soong's graph.
        'libar-pal',
        # libqtigefar links the V2/V3 audio interfaces; libaudioplatformconverter.qti links
        # the V4 ones, so declaring the edge puts both generations of the same interface in
        # one soong graph. The converter is built from source by
        # hardware/qcom-caf/sm8750/audio/primary-hal and installed to vendor/lib64, so the
        # runtime link is unaffected -- only the declared graph changes.
        'libaudioplatformconverter.qti',
        # android.hardware.bluetooth.audio_sw links the V2/V3 audio interfaces;
        # libaudioserviceexampleimpl pulls bluetooth.audio-V5 and with it audio.common-V4.
        # It is built from source by hardware/interfaces/audio/aidl/default and installed to
        # vendor/lib64, so only the declared graph changes.
        'libaudioserviceexampleimpl',
        # libspukeymintprovision links keymint-V2; libspukeymint links keymint-V3, so
        # declaring the edge puts both in one soong graph. libspukeymint is shipped as its
        # own blob and keeps declaring V3, so both versions are still built and installed
        # and the runtime link is unchanged.
        'libspukeymint',
        # libwfdmmsrc_proprietary (WiFi Display) links media.audio.common.types-V2, but the
        # platform's frozen audio.core-V2 interface pulls types-V3, so no single version
